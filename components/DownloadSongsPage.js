...
const DownloadSongsPage = () => {
  ...
  // useEffect để lấy dữ liệu bài hát đã tải về từ backend nếu cần
  useEffect(() => {
    const fetchSongs = async () => {
      try {
        ...
        if (user?.profile?.songDownloaded) {
          // Lọc các bài hát đã tải về và cập nhật vào Redux
          const downloadedSongsData = data.filter((song) =>
            user.profile.songDownloaded.includes(song.id)
          );
          dispatch(setDownloadedSongs(downloadedSongsData)); // Cập nhật bài hát đã tải về vào Redux
        }
      } catch (error) {
...
export default DownloadSongsPage;
